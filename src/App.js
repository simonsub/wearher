...
import { weatherForecast, weatherForecastGetName } from "./Api";
import Weather from "./components/Weather";
import Loader from "./components/Loader";
import geoloc from "./assets/geoloc.svg";
...
function App() {
  ...
  function success(position) {
    ...
    axios.get(weatherForecastGetName(lat, lon)).then((response) => {
      const data = response.data;
      state.value = data.name;
      handleSearchCity();
    });
  }
  const handleSearchNameCity = () => {
    setState({
      ...state,
      loading: true,
    });
  ...
  return (
    <>
      <Search
        ...
        submit={handleSearchInput}
      />
      <div className="geoContainer">
        <button onClick={handleSearchNameCity} className="geoButton">
          <img src={geoloc} alt="Geolocation" />
        </button>
      </div>
      {state.loading === true ? (
        <Loader />
